...
Change `ADMIN_PASSWORD` / `ADMIN_PASSWORD_HASH`, `SESSION_SECRET`, and the MySQL password before production. For managed MySQL, set `MYSQL_SSL=true` (TLS is on; provider self-signed chains are accepted unless you set `MYSQL_SSL_CA` or `MYSQL_SSL_REJECT_UNAUTHORIZED=true`).
